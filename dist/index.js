...
var Editable = props => {
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    if (hasDomSelection && newDomRange && (isRangeEqual(domSelection.getRangeAt(0), newDomRange) || selection && !slate.Range.isCollapsed(selection))) {
      return;
    ...
    if (newDomRange) {
      ...
      if (selection && slate.Range.isCollapsed(selection) && leafEl.hasAttribute('data-slate-zero-width') && leafEl.getAttribute('data-slate-zero-width') !== 'z') {
...
var setFragmentData = (dataTransfer, editor) => {
  var {
    selection
  } = editor;
  ...
  var fragment = slate.Node.fragment(editor, selection);
...
var ReactEditor = {
  ...
  hasDOMNode(editor, target) {
    ...
    return targetEl.closest("[data-slate-editor]") === editorEl && (!editable || targetEl.isContentEditable || targetEl.hasAttribute('data-grid-element') || !!targetEl.getAttribute('data-slate-zero-width'));
  },
  ...
  toDOMRange(editor, range) {
    ...
    var [startNode, startOffset] = slate.Range.isBackward(range) ? domFocus : domAnchor;
    var [endNode, endOffset] = slate.Range.isBackward(range) ? domAnchor : domFocus; // A slate Point pointing to a Leaf without text will always have an offset of 0.
